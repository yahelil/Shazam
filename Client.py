...
class Client:
    def __init__(self):
        ...
        self.prot = protocol.Protocol(self.client_socket)
        self.root = None
    ...
    def register(self, client_name="Client1", password="1678"):
        ...
        msg = f"REGISTER {client_name} {password}".encode('utf-8')
        msg = self.prot.create_msg(msg)
        self.client_socket.send(msg)

        # Receive server response
        response = self.prot.get_msg()[1]
    ...
    def main_window(self):
        ...
        self.sign_up_image=ImageTk.PhotoImage(image)
        image = Image.open("guiimages/sign-in.png")
        image = image.resize((150,100), resample=Image.BICUBIC)
        self.sign_in_image = ImageTk.PhotoImage(image)

        # Create and place the buttons

        register_button = tk.Button(f3, text="Sign up", image=self.sign_up_image, command=self.combine_funcs(self.on_register, self.root.withdraw), pady=10)
        register_button.pack(side=tk.LEFT, padx=25, pady=20)

        assign_button = tk.Button(f3, text="Sign in", image=self.sign_in_image, command=self.combine_funcs(self.root.withdraw, self.on_assign), padx=10, pady=10)
